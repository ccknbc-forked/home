(function () {
	...
	function init() {
		...
		addLine(`<span class="time">${time.getFullYear()}-${time.getMonth()+1}-${time.getDate()}</span> <span class="info">Email:</span><span> ccknbc@outlook.com</span>`)
	...
	var commands = {
		...
		source:function (){
			execCommand('open https://github.com/ccknbc/home')
		},
		weather:function (){
		return 	$.ajax({
				...
				success:function (data){
					...
					addLine(`*********************`);
					addLine(`<span class="info">Tip</span><span>Input 'help' to see help doc!</span>`)
				}
			})
		},
